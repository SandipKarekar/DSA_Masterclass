...
int main(void)
{
    ...
    show_list(p_list, "Showing empty list immediately after_creation:");
    puts(line);
    ...
    show_list(p_list, "Showing list to demostrate that get_end() returns the end data without removing it:");
    puts(line);
    ...
    assert(status == TRUE);
    printf("Exptected return value TRUE from find() for existing data 100\n");
    puts(line);
    ...
    assert(status == SUCCESS);
    p_list = NULL;
    printf("List is destroyed successfuly\n");
    puts(line);
