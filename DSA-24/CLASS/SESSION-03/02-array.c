/*
    ...
    The array is filled in such a way that all negative numbers are stacked on the 
    left side of the array and all non-negative numbers are stacked on the right
    side of the array.
    
    Your job is to right shift and rotate all non-negative elements.

    @boundary case: There may not be nagative element, in that case the entire
    array will be right shifted and rotated.
*/
